...
def update_graph(stock_input, start_date, end_date):
    ...
    for i, sym in enumerate(symbols):
        try:
            ...
            data['Close'] = pd.to_numeric(data[f'Close_{sym}'], errors='coerce').fillna(method='ffill')
            data['Volume'] = pd.to_numeric(data[f'Volume_{sym}'], errors='coerce').fillna(0)
    ...
    if not valid_symbol_found:
        empty_fig = go.Figure()
        empty_fig.update_layout(title="No valid data for the entered symbols")
        return empty_fig, empty_fig
